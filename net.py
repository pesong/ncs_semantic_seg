...
def fcn(split):
    ...
    # 将 pool5， 即fc7之后的数据两倍上采样  1/32  --》 1/16
    ...
    # 将pool4层后的数据进行conv操作       1/16  --》 1/16   通道512 ---》 21
    n.score_pool4 = L.Convolution(n.pool4, num_output=21, kernel_size=1, pad=0,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])

    # 将score_pool4 按照 upscore2 crop  axis=2, offset=5
    ...
    # 将上述sum后的结果再两倍上采样         1/16  ----》 1/8
    n.upscore_pool4 = L.Deconvolution(n.fuse_pool4,
        convolution_param=dict(num_output=21, kernel_size=4, stride=2,
            bias_term=False),
        param=[dict(lr_mult=0)])

    # 将pool3 之后的数据conv操作           1/8    ---》 1/8  通道 ---》 21
