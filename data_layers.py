...
class CityScapeSegDataLayer(caffe.Layer):
    ...
    def load_label(self):
        ...
        label = cv2.imread(self.labellist[self.idx])
        label = cv2.resize(label, (self.cropsize[1], self.cropsize[0]), interpolation=cv2.INTER_CUBIC)
        label = label.astype(np.float32)


        # cv2.imwrite(self.labellist[self.idx] + ".png", label)
        ...
        # label_road = np.all(label == [1], axis=0)
        # label_bg = np.any(label != [1], axis=0)
        ...
        # cv2.imshow("label_all",label_all)
        # cv2.waitKey(0)

        # label_ = label_all[np.newaxis, ...]
        return label
